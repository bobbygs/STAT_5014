...
For me, the colors and styling of a graph are very important.  Sometimes 
...
comp_prop <- function(vec, success = 1, fail = 0) {
  num_success <- sum(vec[vec==success])
  prop <- num_success / length(vec)
  return(prop)
  }
...
set.seed(12345)
P4b_data <- matrix(rbinom(100, 1, prob = (30:40)/100), nrow = 10, ncol = 10)
...
# By cols:
apply(P4b_data, 2, comp_prop)

# By rows:
apply(P4b_data, 1, comp_prop)

```

The columns all have a proportion of 0.6. Each row either has a proportion of 0 or 1. This happens because our call to rbinom only generates a vector of length 10, but we have created a matrix with 100 elements; so R fills the first column with the result from rbinom and then replicates it to fill out the rest of the matrix.  For the reason, each row is made up of all 0s or all 1s, and each column is the exact same vector of 0s and 1s.
...
flip_coins <- function(p = 0.5) {
  flip_vec <- rbinom(n = 10, size = 1, prob = p)
  return(flip_vec)
}

set.seed(12345)
prob_vec <- c((30:40)/100)
flip_mat <- sapply(prob_vec, flip_coins)

result_vec <- apply(flip_mat, 2, comp_prop)
result_labs <- sapply(c((30:40)/100), function(x) paste("P = ", x))
names(result_vec) <- result_labs
knitr::kable(result_vec, caption="Test")

```

It appears that my new function fixed the problem we had when creating the original matrix in part b; however, it is hard to tell what the true proportion within each column were supposed to be.  Since we only did ten simulated flips for each probability, it is not unreasonable that we observed proportions within the columns that varied from the true proportions we set.
